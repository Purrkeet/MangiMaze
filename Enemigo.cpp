...
Enemigo::~Enemigo(){}
void Enemigo::setPosEnemigo(int px, int py){
...
void Enemigo::dibujar(sf::RenderWindow &pmWindow){
	pmWindow.draw(sEnemigo);
}
sf::CircleShape Enemigo::getEnemigoShape(){
	return sEnemigo;
}
void Enemigo::initDj(){
...
ii Enemigo::BuscaPortalMan(ii posPortalInicial, char ColorActual, vii portales, vvi GPortales){ //retorna en X luego Y
	int tPortal = 0;
	switch (ColorActual){
...
bool Enemigo::hayPortal(ii pos, int ** maze){
	if (maze[pos.first][pos.second] == 'A' || maze[pos.first][pos.second] == 'R' || maze[pos.first][pos.second] == 'V')
		return true;
	return false;
}
viii Enemigo::GeneraListaAdyDj(ii nodo, int colM, vii posPortales, vvi GPortales, int **maze){
	viii ady = viii();
...
bool Enemigo::hayParedBFS(ii pos, int** maze){
	if (maze[pos.second][pos.first] != muro)
		return false;
	return true;
}
void Enemigo::Dijkstra(ii nodoI, int ** maze, int col, int fil, int colM, vii Portales, vvi GPortales, vii Puertas){
	initDj();
	// X col Y fil
	...
	while (!Cola.empty()) {
		...
		for (int i = 0; i< sza; ++i) { //-1 no matVisitados, -2 pared , otro numero es el orden
			...
			if (matVisitados[nodoSig.first][nodoSig.second] > matVisitados[posAct.first][posAct.second] + pesoAlNodo){
...
siic Enemigo::PilaHaciaSalidaDJColor(ii pexit, int ** maze, int colM){
...
void Enemigo::CaminoJugadorDJ(ii posJugaodr, int ** maze, int colM, vii Portales, int fil, int col, vvi GPortales, vii Puertas){
	ii posJugadorTI = make_pair(sEnemigo.getGlobalBounds().left, sEnemigo.getGlobalBounds().top);
	//recibe X,Y o sea Col, Fil en el pair
	//usar solo una vez, poner despues de generar matriz
	posJugadorTI.first -= offsetC;
